                ...
                if event.button == 1:
                    ...
                    if grille[un][deux] == '':
                        ...
                        if verification_colonnes():
                            arriere_plan()
                            afficher_symboles()
                            message = f"Le joueur {premier_joueur} a gagné !"
                            resultas_text = font.render(message, True, (0, 0, 0))
                                ...
                                for event in pygame.event.get():
                                    if event.type == pygame.QUIT:
                                        ...
                                        if 50 <= event.pos[0] <= 250 and 20 <= event.pos[1] <= 70:
                                            etat_jeu = "menu"  # Changement de l'état pour retourner au menu
                                            attente = False
                                        # Clic sur le bouton "Quitter"
                            ...
                            afficher_symboles()
                            message = "Match nul !"
                            nouvelle_surface.fill((255, 255, 255))
                            resultas_text = font.render(message, True, (0, 0, 0))
                            resultas_text = pygame.font.Font(None, 30).render(message, True, (100, 200, 230))
                            nouvelle_surface.blit(resultas_text, (fenetre_size // 2 - resultas_text.get_width() // 2, 25))
                            fenetre.blit(nouvelle_surface, (0, 0))
